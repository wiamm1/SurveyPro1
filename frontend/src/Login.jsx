...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const oauthRole = params.get('role') || 'viewer';
      ...
      authService
        ...
        .then((profile) => {
          const nextUser = {
            ...
            email: profile.email,
            name: profile.name,
            role: profile.role || oauthRole,
            companyId: profile.company_id,
            permissions: profile.permissions,
            isActive: profile.is_active,
          };
          setSession(nextUser, oauthToken);
          setUser(nextUser);
  ...
  }, [location.search, navigate, hydrateFromStorage, clearSession, setSession]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const profile = await authService.getMe(response.access_token);
      const nextUser = {
        id: profile.id,
        email: profile.email,
        name: profile.name,
      ...
      setSession(nextUser, response.access_token);
      setUser(nextUser);
      ...
      <div className="w-full max-w-[440px] bg-white border border-slate-100 p-10 rounded-2xl shadow-sm">
        ...
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24">
            ...
            <path
              fill="#34A853"
              d="M12 23c3.24 0 5.97-1.07 7.96-2.91l-3.73-2.89c-1.04.7-2.37 1.12-4.23 1.12-3.22 0-5.89-2.61-6.84-5.67l-3.87 3C3.21 20.27 7.24 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.16 14.71c-.24-.72-.38-1.49-.38-2.31s.14-1.59.38-2.31l-3.87-3C.47 8.77 0 10.33 0 12s.47 3.23 1.29 4.89l3.87-3.18z"
            />
            <path
              fill="#EA4335"
              d="M12 5.04c1.66 0 3.2.57 4.38 1.69l3.27-3.27C17.67 1.54 15.03 1 12 1 7.24 1 3.21 3.73 1.29 7.71l3.87 3C6.11 7.65 8.78 5.04 12 5.04z"
            />
          </svg>
          {text.google}
        </button>
