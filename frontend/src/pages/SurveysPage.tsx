...
export const SurveysPage: React.FC = () => {
  ...
  const chargerEnquetes = async (recherche: string = '') => {
    ...
    try {
      ...
      const reponse = await fetch(
        `${URL_API}?search=${encodeURIComponent(recherche)}`,
        {
          method: 'GET',
          headers: {
      ...
      if (!reponse.ok)
        throw new Error('Erreur lors du chargement des enquêtes');
  ...
  useEffect(() => {
    const delaiDebounce = setTimeout(
      () => chargerEnquetes(termeRecherche),
      300
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Gérez vos enquêtes de satisfaction
          </p>
        </div>

        <div className="flex items-center gap-3">
          <RequirePermission permission="surveys:create" fallback={null}>
            <button
